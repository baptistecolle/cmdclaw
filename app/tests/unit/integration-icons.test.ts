...
describe("integration-icons", () => {
  test("provides icon metadata for every supported integration", () => {
    for (const integration of Object.keys(INTEGRATION_DISPLAY_NAMES)) {
      ...
      expect(getIntegrationDisplayName(integration)).toBe(INTEGRATION_DISPLAY_NAMES[integration as keyof typeof INTEGRATION_DISPLAY_NAMES]);
  ...
  test("normalizes action labels for UI display", () => {
    ...
    expect(linkedinActions.find((item) => item.key === "posts.comment")?.label).toBe("Comment on post");
    expect(linkedinActions.find((item) => item.key === "posts.react")?.label).toBe("React to post");
    expect(linkedinActions.find((item) => item.key === "messages.start")?.label).toBe("Start conversation");
  });
