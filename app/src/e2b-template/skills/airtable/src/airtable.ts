...
async function api(path: string, options?: RequestInit) {
  const res = await fetch(`https://api.airtable.com/v0${path}`, { ...options, headers: { ...headers, ...options?.headers } });
...
async function listBases() {
  ...
  const bases = data.bases.map((b: any) => ({ id: b.id, name: b.name, permission: b.permissionLevel }));
  console.log(JSON.stringify(bases, null, 2));
}

async function getSchema() {
  if (!values.base) { console.error("Required: --base <baseId>"); process.exit(1); }
...
async function listRecords() {
  if (!values.base || !values.table) {
    console.error("Required: --base <baseId> --table <tableIdOrName>"); process.exit(1);
  }
  ...
  const data = await api(`/${values.base}/${encodeURIComponent(values.table)}?${params}`);
...
async function getRecord() {
  if (!values.base || !values.table || !values.record) {
    console.error("Required: --base <baseId> --table <table> --record <recordId>"); process.exit(1);
  }

  const record = await api(`/${values.base}/${encodeURIComponent(values.table)}/${values.record}`);
  console.log(JSON.stringify({ id: record.id, ...record.fields }, null, 2));
}

async function createRecord() {
  if (!values.base || !values.table || !values.fields) {
    console.error("Required: --base <baseId> --table <table> --fields '<json>'"); process.exit(1);
  }

  const fields = JSON.parse(values.fields);
  const record = await api(`/${values.base}/${encodeURIComponent(values.table)}`, {
...
async function updateRecord() {
  if (!values.base || !values.table || !values.record || !values.fields) {
    console.error("Required: --base <baseId> --table <table> --record <recordId> --fields '<json>'");
    process.exit(1);
  }
...
async function deleteRecord() {
  ...
  const result = await api(`/${values.base}/${encodeURIComponent(values.table)}/${values.record}`, { method: "DELETE" });
  console.log(`Record ${result.id} deleted.`);
}

async function searchRecords() {
  if (!values.base || !values.table || !values.search || !values["search-field"]) {
    console.error("Required: --base <baseId> --table <table> --search <value> --search-field <fieldName>");
    process.exit(1);
  }

  const formula = `SEARCH("${values.search}", {${values["search-field"]}})`;
  const params = new URLSearchParams({ filterByFormula: formula, maxRecords: values.limit || "10" });
...
async function main() {
  ...
  try {
    switch (command) {
      ...
      case "schema": await getSchema(); break;
      case "list": await listRecords(); break;
      case "get": await getRecord(); break;
      case "create": await createRecord(); break;
      case "update": await updateRecord(); break;
